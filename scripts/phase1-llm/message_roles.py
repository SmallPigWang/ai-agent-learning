...
def validate_roles(messages: list[dict]) -> bool:
    ...
    for msg in messages:
        if (
            not isinstance(msg, dict) or "role" not in msg
        ):  # 检查是否类型为dict以及字段是否包含role
            return False
    ...
    if roles.count("system") > 1:
        return False
    return not ("system" in roles and roles[0] != "system")

    return True


def chat_with_system(
    system_prompt: str, history: list[dict], user_msg: str, api_key: str
) -> str:
    """
...
if __name__ == "__main__":
    ...
    print(
        f"{'PASS' if r1 else 'FAIL'} validate_roles(合法4角色) -> {r1} | expected: True"
    )
    ...
    print(
        f"{'PASS' if not r2 else 'FAIL'} validate_roles(非法role) -> {r2} | expected: False"
    )
    ...
    print(
        f"{'PASS' if not r3 else 'FAIL'} validate_roles(system不在首位) -> {r3} | expected: False"
    )
    ...
    print(
        f"{'PASS' if not r4 else 'FAIL'} validate_roles(重复system) -> {r4} | expected: False"
    )
    ...
    reply5 = chat_with_system(
        "你是一位严格的英语老师，用英文回答所有问题", [], "早上好怎么说？", key
    )
    print(
        f"{'PASS' if 'good morning' in reply5.lower() else 'FAIL'} "
        f"chat_with_system(英语老师) -> {reply5!r} | expected: 含 good morning"
    )
    all_pass = all_pass and ("good morning" in reply5.lower())

    # 测试6: system 设定角色 — 翻译官
    reply6 = chat_with_system(
        "你是一个中译英翻译官，只输出英文译文", [], "我喜欢学习编程", key
    )
    print(
        f"{'PASS' if reply6 and all(ord(c) < 128 for c in reply6) else 'FAIL'} "
    ...
    print(
        f"{'PASS' if '小红' in reply7 else 'FAIL'} "
