...
def main() -> None:
    ...
    if start_match:
        ...
        days = (date.today() - start_date).days + 1  # noqa: DTZ011
    ...
    print(f"   📅 {datetime.now().strftime('%Y-%m-%d %H:%M')}")  # noqa: DTZ005
    print(f"   📆 已学习 {days} 天")
    print("=" * 60)

    for name, done in phase_done.items():
        bar = progress_bar(done, phase_total[name])
        print(f"  {name:<20} {bar}  ({done}/{phase_total[name]})")
    ...
    # 下一步建议
    for name, done in phase_done.items():
        if phase_total[name] > 0 and done < phase_total[name]:
            remaining = phase_total[name] - done
            print(f"\n  👉 下一步: {name} 还有 {remaining} 项待完成")
            break
