...
if __name__ == "__main__":
    ...
    if e1.name == "张三" and e1.emp_id == "E001" and e1.salary == 8000:
        ...
        print(f"FAIL Employee属性 -> name={e1.name}, id={e1.emp_id}, salary={e1.salary}")
        all_pass = False
    ...
    if repr(e1) == "张三 (E001) - ¥8000/月":
        print(f"PASS Employee.__repr__ -> {repr(e1)}")
    else:
        print(f"FAIL Employee.__repr__ -> {repr(e1)} | expected: 张三 (E001) - ¥8000/月")
        all_pass = False

    # 测试4: Manager 继承属性
    m1 = Manager("李四", "M001", 15000, team_size=5)
    if m1.name == "李四" and m1.emp_id == "M001" and m1.salary == 15000 and m1.team_size == 5:
        print(f"PASS Manager继承属性 -> {m1.name}, team_size={m1.team_size}")
    else:
        print(f"FAIL Manager继承属性 -> {m1.name}, {m1.emp_id}, {m1.salary}, team_size={m1.team_size}")
        all_pass = False

    # 测试5: Manager 默认 team_size
    m2 = Manager("王五", "M002", 12000)
    if m2.team_size == 0:
        print(f"PASS Manager默认team_size -> 0")
    else:
    ...
    if repr(m1) == "李四 (M001) - Manager | ¥15000/月 | team: 5人":
        print(f"PASS Manager.__repr__ -> {repr(m1)}")
    else:
        print(f"FAIL Manager.__repr__ -> {repr(m1)} | expected: 李四 (M001) - Manager | ¥15000/月 | team: 5人")
        all_pass = False
    ...
    if d1.skills == ["JavaScript", "TypeScript"]:
        ...
        print(f"FAIL Developer添加技能 -> {d1.skills} | expected: ['JavaScript', 'TypeScript']")
        all_pass = False

    # 测试13: Developer.__repr__()
    if repr(d2) == "钱七 (D002) - Developer | ¥18000/月 | skills: [Python, Java]":
        print(f"PASS Developer.__repr__ -> {repr(d2)}")
    else:
        print(f"FAIL Developer.__repr__ -> {repr(d2)} | expected: 钱七 (D002) - Developer | ¥18000/月 | skills: [Python, Java]")
        all_pass = False

    # 测试14: isinstance 验证继承关系
    if isinstance(m1, Employee) and isinstance(d2, Employee):
        print(f"PASS 继承链验证 -> Manager和Developer都是Employee的子类")
    else:
        print(f"FAIL 继承链验证")
        all_pass = False
